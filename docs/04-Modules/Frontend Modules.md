...
All visible values continue to come from the real audit object. Sample names, revenue-loss estimates, ratings, review percentages, social statistics, and technical scores embedded in the design reference are never rendered as restaurant facts. Because the current audit contract does not contain verified restaurant revenue, the reference's dollar-loss banner is rendered as a highest-impact opportunity banner instead of publishing an illustrative currency claim. Review sentiment, response behavior, and topic themes retain their existing evidence thresholds. Unsupported sections and unknown metrics remain hidden rather than being filled with placeholders.
