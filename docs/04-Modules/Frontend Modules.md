...
The component deliberately hides many provider plumbing failures from the owner-facing narrative while lower-level diagnostics remain in the returned audit object. Its visual system follows the approved `Audit Report Redesign.html` reference as a structural contract, not merely a palette reference. The owner sees, in order: the Tossdown navigation, restaurant hero and score gauge, a high-impact opportunity banner, biggest leak and three priorities, growth-pillar cards, a featured website-and-ordering card, customer-voice evidence, social-channel evidence, supporting technical evidence, the grouped direct-channel checklist, and the final recommended actions. The layout uses the reference's warm cream canvas, focused 880px reading column, white sand-bordered cards, compact evidence tables, Tossdown-pink action rows, and responsive one-column mobile treatment.

All visible values continue to come from the real audit object. Sample names, currency amounts, ratings, review percentages, social statistics, and technical scores embedded in the design reference are never rendered as restaurant facts. Because the current audit contract does not contain verified restaurant revenue, the reference's dollar-loss banner reports a directional **potential revenue exposure range** rather than a currency claim. The model weights verified pillar gaps by their relationship to revenue (`websiteOrdering` 35%, `reputation` 20%, `retention` 20%, `measurement` 15%, and `engagement` 10%), normalizes across the scored pillars, converts 30% of that weighted gap into an exposure midpoint, and shows a rounded uncertainty range adjusted by audit coverage. The UI explicitly says this is a directional model based on public customer-journey gaps, not verified sales or accounting data.

Review sentiment, response behavior, and topic themes retain their existing evidence thresholds. Unsupported sections and unknown metrics remain hidden rather than being filled with placeholders. Checklist totals are rendered as two self-contained summary pills so each number remains visually attached to its `confirmed` or `needs attention` label at every viewport width.
